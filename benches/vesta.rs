...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function(
        "anemoi-jive/vesta/8-7 (128 bits security) - 8-to-1 compression",
        |bench| {
            let v = [Felt::one(); anemoi_8_7::STATE_WIDTH];
    ...
    c.bench_function(
        "anemoi-jive/vesta/10-9 (128 bits security) - 2-to-1 compression",
        |bench| {
    ...
    c.bench_function(
        "anemoi-jive/vesta/10-9 (128 bits security) - 10-to-1 compression",
        |bench| {
            let v = [Felt::one(); anemoi_10_9::STATE_WIDTH];
    ...
    c.bench_function(
        "anemoi-jive/vesta/12-11 (128 bits security) - 2-to-1 compression",
        |bench| {
    ...
    c.bench_function(
        "anemoi-jive/vesta/12-11 (128 bits security) - 12-to-1 compression",
        |bench| {
            let v = [Felt::one(); anemoi_12_11::STATE_WIDTH];
    ...
    c.bench_function(
        "anemoi-sponge/vesta/8-7 (128 bits security) - hash 10KB",
        |bench| {
            let mut data = vec![0u8; 10 * 1024];
            let mut rng = OsRng;
            rng.fill_bytes(&mut data);
    ...
    c.bench_function(
        "anemoi-sponge/vesta/10_9 (128 bits security) - hash 10KB",
        |bench| {
            let mut data = vec![0u8; 10 * 1024];
            let mut rng = OsRng;
            rng.fill_bytes(&mut data);
    ...
    c.bench_function(
        "anemoi-sponge/vesta/12-11 (128 bits security) - hash 10KB",
        |bench| {
            let mut data = vec![0u8; 10 * 1024];
            let mut rng = OsRng;
            rng.fill_bytes(&mut data);
