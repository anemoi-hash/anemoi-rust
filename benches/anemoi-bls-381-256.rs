...
fn criterion_benchmark(c: &mut Criterion) {
    c.bench_function(
        "anemoi-jive/bls12_381/8-7 (256 bits security) - 2-to-1 compression",
        |bench| {
    ...
    c.bench_function(
        "anemoi-jive/bls12_381/8-7 (256 bits security) - 4-to-1 compression",
        |bench| {
            let v = [Felt::one(); anemoi_8_7_256::STATE_WIDTH];
    ...
    c.bench_function(
        "anemoi-jive/bls12_381/12-11 (256 bits security) - 2-to-1 compression",
        |bench| {
    ...
    c.bench_function(
        "anemoi-jive/bls12_381/12-11 (256 bits security) - 6-to-1 compression",
        |bench| {
            let v = [Felt::one(); anemoi_12_11_256::STATE_WIDTH];
    ...
    c.bench_function(
        "anemoi-sponge/bls12_381/2-1 (256 bits security) - hash 10KB",
        |bench| {
    ...
    c.bench_function(
        "anemoi-sponge/bls12_381/8-7 (256 bits security) - hash 10KB",
        |bench| {
            let mut data = vec![0u8; 10 * 1024];
            let mut rng = OsRng;
            rng.fill_bytes(&mut data);
    ...
    c.bench_function(
        "anemoi-sponge/bls12_381/12-11 (256 bits security) - hash 10KB",
        |bench| {
            let mut data = vec![0u8; 10 * 1024];
            let mut rng = OsRng;
            rng.fill_bytes(&mut data);
